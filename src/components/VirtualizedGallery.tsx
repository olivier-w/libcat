...
const LOW_POWER_ITEM_MIN_WIDTH = 220
      ...
      <div style={adjustedStyle}>
        <div style={innerStyle}>
          <div
            ...
            className={`movie-card relative w-full h-full rounded-xl overflow-hidden cursor-pointer border border-smoke-900/30 bg-obsidian-400/25 ${
              isSelected ? 'movie-card-selected' : ''
            }`}
            style={{
              borderColor: isSelected ? 'rgba(217, 149, 110, 0.8)' : 'rgba(45, 43, 50, 0.25)',
            }}
          >
            <div className="thumbnail-container">
              ...
              {posterUrl ? (
                <img
                  ...
                  loading="lazy"
                  decoding="async"
                  width={Math.max(1, Math.round(columnWidth - ITEM_GAP))}
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-obsidian-950/85 via-obsidian-950/30 to-transparent pointer-events-none" />
            ...
            <button
              ...
              className={`absolute top-2.5 right-2.5 w-8 h-8 rounded-lg flex items-center justify-center ${
                movie.favorite
                  ? 'bg-cinnabar-500/80 text-white'
                  : 'bg-obsidian-950/80 text-pearl-300/80'
              }`}
            >
              <svg
                className="w-4 h-4"
            ...
            {movie.watched && (
              <div className="absolute top-2.5 right-12 w-8 h-8 rounded-lg flex items-center justify-center bg-emerald-500/80 text-white">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-3 bg-obsidian-950/55 pointer-events-none">
              <h3 className="text-sm font-medium text-pearl-100 line-clamp-2">
                {movie.title || 'Untitled'}
              </h3>
              {movie.year && (
            ...
            {isSelected && (
              <div className="absolute inset-0 rounded-xl border-2 border-bronze-400 pointer-events-none" />
            )}
          </div>
        </div>
      </div>
...
export function VirtualizedGallery({ movies }: VirtualizedGalleryProps) {
  ...
  const { selectedMovies, selectedIds, setSelectedMovie, updateMovieInState, removeMoviesFromState } = useLibraryStore()
  ...
  const itemMinWidth = lowPowerEnabled ? LOW_POWER_ITEM_MIN_WIDTH : ITEM_MIN_WIDTH
  const columnCount = Math.max(1, Math.floor((contentWidth + ITEM_GAP) / (itemMinWidth + ITEM_GAP)))
